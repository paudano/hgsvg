...
plot(c(), xlim=c(0,shortMax),ylim=c(0,max(colSum)), ylab="Count", xlab="Structural variant length (bp)", main=paste(args$sample, opTitle, " < 1000 bp", sep=" "))
...
print("long hist")
plot(c(), xlim=c(0,longMax),ylim=c(0,max(colSum)), ylab="Count", xlab="Structural variant length (bp)",main=paste(args$sample, opTitle, " >= 1000, < 20,000 bp", sep=" "))
